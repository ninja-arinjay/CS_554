...
export default function attractions({data, id:page}) {
  return (
    <Layout>
      <Head>
      ...
      {page > 1 && (
        <Link href={`/attractions/page/${parseInt(page)-1}`}>
          <a>Previous</a>
        </Link>
      )}
      {page <= 48 && (
...
async function getShowData(id) {
  ...
  try{
    const t = await axios.get(`https://app.ticketmaster.com/discovery/v2/attractions.json?countryCode=US&apikey=eVZDZZnM6njxAC220G8fcwdEanYOm1eJ&page=${id}`);
    return t.data._embedded.attractions;
  }
  catch(e){
    return undefined;
  }
